...
It is always fine to exclude all rectangles that are big enough to enclode the queried area from the get-go, since these rectangles are too large to be fully contained by the queried area and therefore never need to be counted.
...
are always opened with this parameterset X. 
...
The size of the vectors is not stored and needs to be inferred from the file size and object size.


| file | object | desc |
|------|--------|------|
| .desc | char | The description strings of all points. Each description is EOF terminated, so that Points only need to store pointers to the start of their description. Strings are stored as individual characters (1 byte a piece). |
...
- its position td::D sized array of values of type td::coordinate_t.
...
- if td::IS_ORTHOTOPE, each add_point call adds all corners of the described orthotope. \
  In this case each of the added points also stores a value of type uint8_t that indicates which corner. \
...
- td::D * (td::D - 1) many **sparse_coordinate_vector_interval** s. \
  ...
  Each block of td::D - 1 intervals belong to one dimension *n* of the overlay and store the translation to sparse space for all other *n-1* dimensions.
...
All sorted coordinate translation must have the same start and end indices.
