...
template <size_t N> class CacheVector
{
  public:
    ...
    CacheVector( std::string sFilename, Points vvfPoints, std::array<std::vector<int64_t>, N> vBins, size_t uiSizeCache,
                 size_t uiThreads, size_t uiUntouchedDimensions, size_t uiLayer, size_t uiEleNoCache );
    CacheVector( std::string sFilename, size_t uiSizeCache, size_t uiThreads, size_t uiUntouchedDimensions,
                 size_t uiLayer, size_t uiEleNoCache );
  ...
  private:
    ...
    template <typename VEC_T> static void vectorToFile( std::string sFilename, VEC_T& vector )
    {
    ...
    template <typename VEC_T> static void fileToVector( std::string sFilename, VEC_T& vector )
    {
        // open the file:
        ...
        if( iDim >= vBins.size( ) )
        {
            ...
            for( size_t uiI : vBins[ iFixedDim ] )
            {
                ...
                DEBUG( std::cout << "integrate " << pointCoords( vPoint ) << " (" << uiIdx << "/" << vData.size( )
                                 << ")"
                                 << ": " << vData[ uiIdx ] << " -> " << uiIntegral << std::endl
                                 << std::endl; )
    ...
    void makeCache( size_t uiI, Point& vP )
    {
        ...
        if( b )
            ...
            vDataCache[ uiI ] =
                std::make_shared<CacheVector>( sNewFileName, vDataPoints[ uiI ], makeBins( uiI ), uiSizeCache,
    ...
    int64_t count_cache( Point& vP, Point vLeft, Point vRight, size_t i )
    {
        ...
        else
        {
            ...
            if( xStart != vBins[ i ].begin( ) )
                xStart--;
            for( auto itI = xStart; itI != std::upper_bound( vBins[ i ].begin( ), vBins[ i ].end( ), vRight[ i ] );
                 ++itI )
            {
                vP[ i ] = *itI;
  ...
  public:
    ...
    std::string print_help( Point& vP, size_t uiDim )
    {
        std::string sRet = "";
        if( uiDim >= vBins.size( ) )
            sRet = pointCoords( vP ) + ": " + std::to_string( vDataPoints[ getIdxPoints( vP, false ) ].size( ) ) +
            ...
            xPool.enqueue(
                [ & ]( size_t, size_t uiI ) {
                    for( size_t uiX = uiI; uiX < vvfPoints.size( ); uiX += uiT )
                    {
                        ...
                        DEBUG( std::cout << "loading " << pointCoords( p.first ) << " "
                                         << getIdxPoints( p.first, false ) << " " << vDataPoints.size( ) << std::endl; )
...
template <size_t N>
CacheVector<N>::CacheVector( std::string sFileName, size_t uiSizeCache, size_t uiThreads, size_t uiUntouchedDimensions,
                             size_t uiLayer, size_t uiEleNoCache )
    : sFilename( sFileName ),
      vBins( ),
      uiUntouchedDimensions( uiUntouchedDimensions ),
...
template <size_t N> typename std::vector<std::deque<std::shared_ptr<CacheVector<N>>>> CacheVector<N>::vpCache;
template <size_t N> typename std::mutex CacheVector<N>::xCacheLock;

} // namespace kdtree

template <size_t N> void export_( pybind11::module& m, std::string sName )
{
    pybind11::class_<kdtree::CacheVector<N>>( m, sName.c_str( ) )
        .def( pybind11::init<std::string, typename kdtree::CacheVector<N>::Points, std::array<std::vector<int64_t>, N>,
