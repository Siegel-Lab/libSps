...
template <typename type_defs> class Dataset
{
  ...
  public:
                                   ...
                                   pos_t uiNumOverlays, pos_t uiCoordinateSizes, pos_t uiMinPos )
    {
        ...
        uint64_t uiNumOverlaySamples = (uint64_t)std::log2( uiNumOverlaysTotal );
        uint64_t uiNumPointSamples =
            (uint64_t)std::log2( xSortedPoints[ 0 ].uiEndIndex - xSortedPoints[ 0 ].uiStartIndex );
    ...
    pos_t actualFromGridPos( pos_t vPos ) const
    {
        pos_t vRet;
        for( size_t uiI = 0; uiI < D; uiI++ )
    ...
    pos_t actualTopRightFromGridPos( pos_t vPos ) const
    {
        for( size_t uiI = 0; uiI < D; uiI++ )
            ++vPos[ uiI ];
